...
@Component({
  selector: 'app-image-pmfu',
  standalone: true,
  imports: [MatDialogModule, CommonModule, MatIconModule],
  templateUrl: './image-pmfu.component.html',
  styleUrl: './image-pmfu.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ImagePmfuComponent {
  images: string[];
  currentIndex: number = 0;
  constructor(
    public dialogRef: MatDialogRef<ImagePmfuComponent>,
    public loginService: LoginService,
    private http: HttpClient,
    @Inject(MAT_DIALOG_DATA) public data: DialogData // Inject MAT_DIALOG_DATA to access the passed data
  ) {
    this.images =
      data.images.map((path) => 'http://localhost:8887/uploads/' + path) || [];
    // Si une image est sélectionnée au départ
    data.selected = 'http://localhost:8887/uploads/' + data.selected;
