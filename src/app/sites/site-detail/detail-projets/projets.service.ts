...
import {
  ...
  OperationLite,
  OperationCheckbox,
} from './projet/operation/operations';
...
export class ProjetService {
  ...
  deleteOperation(uuid_ope: string): Observable<ApiResponse> {
    return this.http
      .delete<ApiResponse>(
        `${this.activeUrl}delete/opegerer.operations/uuid_ope=${uuid_ope}`
      )
      .pipe(
        catchError((error) => {
  ...
  deleteProjet(uuid_proj: string): Observable<ApiResponse> {
    return this.http
      .delete<ApiResponse>(
        `${this.activeUrl}delete/opegerer.projets/uuid_proj=${uuid_proj}`
      )
      .pipe(
        catchError((error) => {
          console.error("Erreur lors de la suppression de l'opération:", error);
   ...
   */
  insertCheckbox(
    checkBoxList: OperationCheckbox,
    table: string
  ...
  deleteCheckbox(
    ...
    return this.http
      .delete<ApiResponse>(
        `${this.activeUrl}delete/opegerer.${table}/${keyName}=${uuid}/checkbox_id/${programme_id}`
      )
      .pipe(
        catchError((error) => {
  ...
  uploadShapefile(formData: FormData): Observable<ApiResponse> {
    ...
    return this.http.post<ApiResponse>(url, formData).pipe(
      catchError((error) => {
        const errorMessage =
          error.error?.message || "Erreur lors de l'import du shapefile";
  ...
  uploadDocfile(formData: FormData): Observable<ApiResponse> {
    ...
    return this.http.put<ApiResponse>(url, formData).pipe(
      catchError((error) => {
        const errorMessage =
          error.error?.message || "Erreur lors de l'import du docfile";
  ...
  deleteItem(
    type: DeleteItemTypeEnum,
    operation?: void | Operation,
    localisations?: void | Localisation[],
    projet?: void | Projet,
    objectif?: void | Objectif
  ): Observable<boolean> {
    ...
    if (type === 'localisation' && localisations && localisations.length > 0) {
      ...
      return new Observable<boolean>((observer) => {
        forkJoin(deleteObservables).subscribe((results) => {
          ...
          if (allSuccess) {
            ...
            this.snackbarService.error(
              'Une ou plusieurs suppressions ont échoué'
            );
            observer.next(false);
          }
          observer.complete();
        ...
        this.deleteProjet(projetId).subscribe(
          (response: ApiResponse) => {
            if (response.success) {
              ...
              const message =
                response.message || 'Erreur lors de la suppression du projet';
              this.snackbarService.error(message);
              console.error(message);
              observer.next(false);
            }
      ...
      return new Observable<boolean>((observer) => {
        this.http
          .delete<ApiResponse>(
            `${this.activeUrl}delete/opegerer.objectifs/uuid_objectif=${objectifId}`
          )
          .subscribe(
            (response: ApiResponse) => {
              if (response.success) {
